    ...
    <Link href={`/book/${id}`} className="h-max w-max select-none">
      <button
        ...
        className={classNames(
          "group box-border flex h-[95px] w-[80px] flex-col items-center justify-start gap-4 rounded-6 bg-white px-12 py-8 text-gray-700 duration-200 focus:outline-none dark:bg-gray-700 dark:text-white",
          isShow &&
            "hover:bg-gray-100 active:bg-gray-200 dark:hover:bg-gray-800 dark:active:bg-gray-900"
        )}
        onClick={isShow ? onClick : undefined}
      >
        <ProfileImage isShow={isShow} className="h-[57px] w-[57px]" src={src} />
